...
import { startMultiFileChat, getGeneralResponse } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleSendMessage = useCallback(async (message: string) => {
    ...
    try {
      ...
      const aiMessage: Message = { sender: 'ai', text: responseText, type: 'document' };
      setMessages(prev => [...prev, aiMessage]);
    } catch (e) {
      console.error(e);
      const errorMessage = 'عذراً، حدث خطأ أثناء التواصل مع الذكاء الاصطناعي. يرجى المحاولة مرة أخرى.';
      setMessages(prev => [...prev, { sender: 'ai', text: errorMessage, type: 'document' }]);
    } finally {
      setIsLoading(false);
    }
  }, [chat]);
  ...
  const handleGetGeneralAnswer = useCallback(async (question: string) => {
    ...
    setError('');
    try {
      const responseText = await getGeneralResponse(question);
      const aiMessage: Message = { sender: 'ai', text: `**إجابة عامة:**\n\n${responseText}`, type: 'general' };
      setMessages(prev => [...prev, aiMessage]);
    } catch (e) {
      console.error(e);
      const errorMessage = 'عذراً، حدث خطأ أثناء الحصول على إجابة عامة. يرجى المحاولة مرة أخرى.';
      setMessages(prev => [...prev, { sender: 'ai', text: errorMessage, type: 'general' }]);
    } finally {
      setIsLoading(false);
    }
  }, []);
